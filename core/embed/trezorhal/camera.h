...
#define WIN_W \
  480  // 接收的摄像头数据尺寸 宽度 单位：像素（对应初始化序列，不可随意修改）
#define WIN_H \
  480  // 接收的摄像头数据尺寸 高度 单位：像素（对应初始化序列，不可随意修改）
 ...
 * 返回: 1：错误
 */
unsigned char camera_sccb_write_reg(unsigned char reg_addr,
                                    unsigned char* data);
 ...
 * 获取摄像头模组 进入休眠模式
 */
#define camera_enter_sleep_mode() \
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_11, GPIO_PIN_SET)
/*
 * 获取摄像头模组 退出休眠模式/进入正常工作模式
 */
#define camera_exit_sleep_mode() \
...
#define cam_buf \
  (unsigned short*)FMC_SDRAM_BOOLOADER_BUFFER_ADDRESS  // SDRAM BUFFER
